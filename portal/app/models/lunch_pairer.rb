class LunchPairer
	...
	def greedy_pairer(list_of_users)
		list_of_pairs = []
		while exist_pairable_user(list_of_users)
			...
			list_of_pairs.sort_by{|user_array| user_array.second.count}
		end
	end

	def exist_pairable_user(list_of_users)
	...
	def clear_user_list(user_arry, list_of_users)
		user_to_clear = user_array.first
		matches_to_clear = user_array.second
		list_of_users.each do |user_array|
			if matches_to_clear.include(user_array.first)
				user.second.delete(user_to_clear)
				matches_to_clear.delete.user.first
			end
		end
